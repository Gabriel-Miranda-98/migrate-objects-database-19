  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "ARTERH"."ARTIGOS_SIMULACAO_APOSENT" ("ANO_MES_REFERENCIA", "CODIGO_PLANO", "CODIGO_EMPRESA", "MATRICULA", "DIGITO", "NOME", "CPF", "PIS_PASEP", "CODIGO_CARGO", "DATA_ADMISSAO_POSSE", "SITUACAO_FUNCIONAL", "TIPO_OCORRENCIA", "DATA_INICIO", "DATA_FIM", "DIAS", "CID", "SEQUENCIAL_CARGO") AS 
  SELECT 
  ...
  TB.DIAS,
  TB.CID,
	(SELECT TBI.SEQUENCIAL_CARGO FROM (
	  SELECT 
			TABELA."CPF_SEGURADO",
			TABELA."COD_CARGO",
		...
		FROM (
			SELECT 
				SUBSTR(PEI.CPF, 1,11)                    AS CPF_SEGURADO,
				SUBSTR(TRIM(COI.COD_CARGO_EFETIVO),12,4) AS COD_CARGO,
				CASE
					WHEN TO_CHAR(COI.DATA_EFETIVO_EXERC,'YYYYMMDD') IS NOT NULL THEN TO_CHAR(COI.DATA_EFETIVO_EXERC,'YYYYMMDD')
					ELSE TO_CHAR(COI.DATA_ADMISSAO,'YYYYMMDD')
				END AS DATA_ADMISSAO_POSSE
			FROM 
				RHPESS_CONTRATO COI 
				LEFT OUTER JOIN RHPESS_PESSOA PEI ON COI.CODIGO_EMPRESA = PEI.CODIGO_EMPRESA
				AND COI.CODIGO_PESSOA = PEI.CODIGO 
				LEFT OUTER JOIN RHPLCS_FUNCAO FU ON COI.CODIGO_EMPRESA = FU.CODIGO_EMPRESA
				AND COI.CODIGO_FUNCAO = FU.CODIGO 
				LEFT OUTER JOIN RHPLCS_CARGO CAI ON COI.CODIGO_EMPRESA  = CAI.CODIGO_EMPRESA
				AND COI.COD_CARGO_EFETIVO = CAI.CODIGO 
				LEFT OUTER JOIN RHORGA_CUSTO_GEREN CG ON COI.CODIGO_EMPRESA = CG.CODIGO_EMPRESA
				AND COI.COD_CUSTO_GERENC1 = CG.COD_CGERENC1
				AND COI.COD_CUSTO_GERENC2 = CG.COD_CGERENC2
				AND COI.COD_CUSTO_GERENC3 = CG.COD_CGERENC3
				AND COI.COD_CUSTO_GERENC4 = CG.COD_CGERENC4
				AND COI.COD_CUSTO_GERENC5 = CG.COD_CGERENC5
				AND COI.COD_CUSTO_GERENC6 = CG.COD_CGERENC6
				LEFT OUTER JOIN RHPARM_SIT_FUNC ST ON COI.SITUACAO_FUNCIONAL = ST.CODIGO
				LEFT OUTER JOIN RHPARM_CAUSA_RESC  ON COI.CAUSA_RESCISAO = RHPARM_CAUSA_RESC.CODIGO
				LEFT OUTER JOIN RHTABS_VINCULO_EMP ON COI.VINCULO = RHTABS_VINCULO_EMP.CODIGO
				LEFT OUTER JOIN RHPLCS_TAB_SALARIO SAL ON CAI.TABELA_UTILIZADA = SAL.CODIGO
				AND CAI.CODIGO_EMPRESA  = SAL.CODIGO_EMPRESA
				LEFT OUTER JOIN RHPONT_ESCALA ESC ON CAI.CODIGO_ESCALA = ESC.CODIGO
				AND CAI.CODIGO_EMPRESA = ESC.CODIGO_EMPRESA
			WHERE COI.CODIGO_EMPRESA = '0001'
				AND COI.TIPO_CONTRATO   IN ('0001')
				AND COI.SITUACAO_FUNCIONAL IN (
						...
						'5300', '5303', '5401', '1310')
				AND COI.VINCULO IN ('0000','0002')
				AND (COI.CODIGO < '000000099999999'
				AND COI.CODIGO NOT IN('000000000777777','000000000823701','000000000833014','000000000866265','000000000747428','000000000190072','000000000420534'))
				/*FALECIDOS PIA*/
				AND COI.ANO_MES_REFERENCIA = (
					SELECT MAX(A.ANO_MES_REFERENCIA)
						FROM RHPESS_CONTRATO A
					 WHERE A.CODIGO = COI.CODIGO
						 AND A.CODIGO_EMPRESA = COI.CODIGO_EMPRESA
						 AND A.TIPO_CONTRATO  = COI.TIPO_CONTRATO
						 AND A.ANO_MES_REFERENCIA <= add_months(sysdate,(-1))-0
				)
				/* CPF para teste*/
			--	AND PEI.CPF = '07085326670'
		) TABELA
		GROUP BY CPF_SEGURADO, COD_CARGO,	DATA_ADMISSAO_POSSE) TBI
		WHERE TBI.CPF_SEGURADO = CPF
		  AND TBI.COD_CARGO = COD_CARGO
			AND TBI.DATA_ADMISSAO_POSSE = DATA_ADMISSAO_POSSE) AS SEQUENCIAL_CARGO	
FROM (  
  SELECT DISTINCT
