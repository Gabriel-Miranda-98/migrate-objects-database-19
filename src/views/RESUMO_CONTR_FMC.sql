  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "ARTERH"."RESUMO_CONTR_FMC" ("ANO_MES_REFERENCIA", "COD_ORG", "TIPO_MOVIMENTO", "MATRICULA", "BASE_CALC_CONTRI", "VAL_CONT_BAS_SEG", "VAL_CONT_PATRO_NORM", "VAL_CONT_ADIC_SEG", "VAL_CONT_PATR_ADIC", "REMUNERACAO_TOTAL", "NOME", "COD_PLANO", "SITUACAO_FUNCIONAL", "DATA_PAGAMENTO", "DECIMO_TERCEIRO", "FERIAS", "RESCISAO") AS 
  SELECT 
    ...
    TO_CHAR (SUM(CASE m.codigo_verba WHEN '4436' THEN m.valor_verba ELSE 0 END), '0000000.00') AS BASE_CALC_CONTRI, 	 --BASE CONTRIB PREVIDENCIARIA GERAL - RPPS
		TO_CHAR (SUM(CASE m.codigo_verba WHEN '4437' THEN m.valor_verba ELSE 0 END), '0000000.00') AS VAL_CONT_BAS_SEG,
    ...
    C.SITUACAO_FUNCIONAL, 
		...
		CASE WHEN m.tipo_movimento = 'DE' OR m.tipo_movimento = 'GD' THEN 'S' ELSE 'N' END AS DECIMO_TERCEIRO,
		'N' AS FERIAS,
