  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "ARTERH"."RESUMO_CONTR_AUDITORIA" ("ANO_MES_REFERENCIA", "COD_ORG", "TIPO_MOVIMENTO", "MATRICULA", "BASE_CALC_CONTRI", "VAL_CONT_BAS_SEG", "VAL_CONT_PATRO_NORM", "VAL_CONT_ADIC_SEG", "VAL_CONT_PATR_ADIC", "REMUNERACAO_TOTAL", "NOME", "COD_PLANO", "SITUACAO_FUNCIONAL", "DATA_PAGAMENTO", "DECIMO_TERCEIRO", "FERIAS", "RESCISAO") AS 
  SELECT    
  to_char(m.ano_mes_referencia,'yyyymm') as ANO_MES_REFERENCIA,    
  C.CODIGO_EMPRESA COD_ORG,
  m.tipo_movimento, 
...
FROM RHPESS_CONTRATO c   
     INNER JOIN RHPESS_PESSOA p ON p.CODIGO_EMPRESA = c.CODIGO_EMPRESA
...
WHERE v.codigo in ('4436', '4437', '4438', '4xx2', '4xx3', '4001') 
  ...
  --and substr(c.codigo, 5,10) = '0000079517'
...
group by m.ano_mes_referencia, c.data_admissao, m.codigo_contrato, c.codigo, m.tipo_movimento, c.situacao_funcional, C.CODIGO_EMPRESA, P.NOME