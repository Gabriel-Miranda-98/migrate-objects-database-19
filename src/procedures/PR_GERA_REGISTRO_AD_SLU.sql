  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_SLU" 
-- *****************************************************************************
...
IS
  ...
  LINHA                      VARCHAR(2000) := '';
  DIRETORIO                  VARCHAR(50)   := '/trab/cdbp05/sirh/arquivos_slu_ad';
  ARQUIVO_INC                VARCHAR(50)   := 'Inclusao_AD_SLU_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR(50)   := 'Alteracao_AD_SLU_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR(50)   := 'Exclusao_AD_SLU_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_TIPO_ARQUIVO             VARCHAR(01)   := 'I';
  V_CONTA_REGISTRO           INTEGER       := 0;

  --VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/SLU';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionários da SLU no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionários da SLU no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionários da SLU no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusões de funcionários da SLU';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alterações de funcionários da SLU';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusões de funcionários da SLU';
...
BEGIN
  ...
  FOR I IN (SELECT 	CNT.CODIGO AS MATRICULA,
                    ...
                    CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'lu'   || SUBSTR(CNT.CODIGO,9,6)  
                         WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'lues' || SUBSTR(CNT.CODIGO,9,6)  
                    ELSE
                      'NÃO DEFINIDA'
                    END LOGIN,
                    CNT.NOME AS NOME,
		    ...
		    AND CNT.CODIGO_EMPRESA        = '0007'
		    AND CNT.TIPO_CONTRATO        IN ('000*1','000*2','000*3','0098')
...
--AND CNT.CODIGO = '000000009800000'
           )
  LOOP
    BEGIN
	  -- ************************************ T E L E F O N E ***********************************************************
