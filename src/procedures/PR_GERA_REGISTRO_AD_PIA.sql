  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_PIA" 
-- *****************************************************************************
...
IS
  ...
  LINHA                      VARCHAR(2000) := '';
  DIRETORIO                  VARCHAR(50)   := '/trab/cdbp05/sirh/arquivos_pbh_ad';
  ARQUIVO_INC                VARCHAR(50)   := 'Inclusao_AD_PBH_PIA_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR(50)   := 'Alteracao_AD_PBH_PIA_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR(50)   := 'Exclusao_AD_PBH_PIA_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ERR                VARCHAR(50)   := 'Erro_AD_PBH_PIA_'     || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_TIPO_ARQUIVO             VARCHAR(01)   := 'I';
  V_CONTA_REGISTRO           INTEGER       := 0;

  --VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/PIA';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionÃ¡rios do PIA da PBH no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionÃ¡rios do PIA da PBH no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionÃ¡rios do PIA da PBH no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusÃµes de funcionÃ¡rios do PIA da PBH';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alteraÃ§Ãµes de funcionÃ¡rios do PIA da PBH';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusÃµes de funcionÃ¡rios do PIA da PBH';
...
BEGIN
  ...
  FOR I IN ( SELECT CNT.CODIGO AS MATRICULA,
                    ...
                    CASE
		              WHEN CNT.COD_CARGO_PAGTO = LPAD('2056',15,0) THEN 'pres' || LOWER(SUBSTR(CNT.CODIGO,7,8))
			          ELSE 'pr' || LOWER(SUBSTR(CNT.CODIGO,9,6))
		            END LOGIN,
                    CNT.NOME AS NOME,
		     ...
		     FROM ARTERH.RHPESS_CONTRATO CNT
			      ...
			      LEFT JOIN  ARTERH.RHORGA_AGRUPADOR U    ON U.COD_AGRUP1  = CNT.COD_CUSTO_CONTAB1
	                                                     AND U.COD_AGRUP2  = CNT.COD_CUSTO_CONTAB2
	                                                     AND U.COD_AGRUP3  = CNT.COD_CUSTO_CONTAB3
	                                                     AND U.COD_AGRUP4  = CNT.COD_CUSTO_CONTAB4
	                                                     AND U.COD_AGRUP5  = CNT.COD_CUSTO_CONTAB5
	                                                     AND U.COD_AGRUP6  = CNT.COD_CUSTO_CONTAB6
	                                                     AND U.CODIGO_EMPRESA = '0001'
                                                         AND U.TIPO_AGRUP = 'C'
             WHERE 1 = 1
	         ...
	         AND CNT.SITUACAO_FUNCIONAL  IN ('1710','1715','5016','5017','5050','5051','6050','6051')		              			 
             ...
             ORDER BY MATRICULA
	       -- REGRAS DAS SITUAÇÕES FUNCIONAIS:
	       -- 1710 RESCISAO IS NULL E TEM AD ATIVO
	       -- 1715 RESCISAO IS NOT NULL E PERDE O AD
	       -- 5016 RESCISAO IS NULL E TEM AD ATIVO 
	       -- 5017 RESCISAO IS NOT NULL E PERDE O AD
	       -- 5050 RESCISAO IS NOT NULL E PERDE O AD
	       -- 5051 RESCISAO IS NOT NULL E PERDE O AD
	       -- 6050 RESCISAO IS NULL E TEM AD ATIVO
	       -- 6051 RESCISAO IS NOT NULL E PERDE O AD
    )
  LOOP
    BEGIN
	  -- ************************************ T E L E F O N E ***********************************************************
	...
	IF I.COD_CARGO_PAGTO = LPAD('2056',15,0) THEN
	  -- ESTAGIÁRIO
      -- SECRETARIA
	  BEGIN      
      ...
      END; -- AQUI
	ELSE
	  V_UNIDADE := 'APOS.INSS/PIA';

	  IF I.SITUACAO_FUNCIONAL IN('1710','5050','5051','6050','6051')
	    THEN V_SECRETARIA := 'PIA-' || SUBSTR(UPPER(I.ABREVIACAO),1,11);
		ELSE V_SECRETARIA := 'APOS-' || SUBSTR(UPPER(I.ABREVIACAO),1,10);
	  END IF;	  
    END IF;
    ...
    ELSE --EXISTE NA TABELA RHPBH_REGISTRO_AD
      --BUSCA NOME, CARGO, SECRETARIA, UNIDADE, DATA_RESCISAO, EMAIL,TELEFONE E IND_OPERACAO DO ÃšLTIMO REGISTRO PARA COMPARAÃ‡AO
      BEGIN -- AQUI
    ...
    IF V_INSERE THEN
      ...
      IF V_QTD_REGISTRO = 0 THEN
	    BEGIN
          ...
          DELETE ARTERH.RHUSER_RL_USR_GRP WHERE LOWER(CODIGO_USUARIO) = I.LOGIN;
 ...
 -- ******************************************************************************************************
  ...
  END LOOP;
  -- ******************************************************************************************************
  -- ******************** ENVIA ARQUIVO TXT DE INCLUSAO E EXCLUSAO POR E-MAIL *****************************
  -- ******************************************************************************************************
  FOR I IN 1..3
  LOOP
    CASE
