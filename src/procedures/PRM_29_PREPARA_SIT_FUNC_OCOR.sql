  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PONTO_ELETRONICO"."PRM_29_PREPARA_SIT_FUNC_OCOR" (DATA_INICIO IN VARCHAR2, DATA_FIM IN VARCHAR2)
AS
BEGIN
--Kellysson em 8/4/22 baseado (preparar_SIT_FUNC_da_OCORRENCIA_v3.sql)

--KELLYSSON EM 7/1/20
--Kellysson em 8/1/21 - Para gravar no Arterh como SITUAÇÃO FUNCIONAL o que for de OCORRENCIA e JUSTIFICATIVA lançadas no Ifponto autorizadas para esse processo baseado tabela de conversão do Arterh IFP2, TRATADA CONVERTENDO DIAS DE JUSTIFICATIVA EM PERIODOS DE SIT FUNC
...
ELSE '-----*********************CENARIO NÃO MAPEADO AINDA'
END CENARIO, 
X3.*
...
LEFT OUTER JOIN (SELECT * FROM PONTO_ELETRONICO.IFPONTO_ESPELHO_HISTORICA WHERE DATA_PROCESSAMENTO IS NULL)E 
...
ELSE
dbms_output.put_line('-----*********************CENARIO NÃO MAPEADO AINDA' );
