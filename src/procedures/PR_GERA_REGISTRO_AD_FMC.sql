  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_FMC" 
-- *****************************************************************************
...
--  0013 -> CULTURA			0001,0003							0001,0003,0098
--  0014 -> PARQUES			0001,0003							0001,0003,0098
...
IS
  ...
  LINHA                      VARCHAR2(2000) := '';
  DIRETORIO                  VARCHAR2(50)   := '/trab/cdbp05/sirh/arquivos_cultura_ad';
  ARQUIVO_INC                VARCHAR2(50)   := 'Inclusao_AD_FMC_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR2(50)   := 'Alteracao_AD_FMC_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR2(50)   := 'Exclusao_AD_FMC_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_CONTA_REGISTRO           INTEGER        := 0;
  V_TIPO_ARQUIVO             VARCHAR(1)     := 'I';

  -- VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/FMC';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionários da FMC no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionários da FMC no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionários da FMC no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusões de funcionários da FMC';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alterações de funcionários da FMC';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusões de funcionários da FMC';
  AV_BODY_HTML               VARCHAR(1)   := 'N';
...
BEGIN
  ...
  FOR I IN (SELECT CNT.CODIGO AS MATRICULA,
           ...
           CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'fc'   || SUBSTR(CNT.CODIGO,9,6)
                WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'fces' || SUBSTR(CNT.CODIGO,9,6)                
           ELSE
             'NÃO DEFINIDA'
           END LOGIN,
           CNT.NOME      	AS NOME,
