  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PONTO_ELETRONICO"."PRG_SIT_PONTO_ORIGEM_ARTE" (DATA_INICIO IN DATE, DATA_FIM IN DATE) AS
BEGIN
...
----v4_v2 em 24/8/21 para ajustes de cessoes e outros casos fora baseado email Lilian Rabelo (Conversão de Situações de ponto para contagem de tempo)
--EM 19/8/20 FICARA PARA OS LOCAIS DA EMPRESA 1 QUE Nï¿½O ESTAO USANDO O FECHAMENTO IFPONTO
--em 13/8/20 VERSaO 5 justificou devido aos fechamentos de apuraï¿½ï¿½o por locais, tendo mais de uma data dentro da PBH
--VERSaO 4 EM 1/6/21 DEVIDO AJUSTES NAS CESSOES E VW_DATAS
...
from ARTERH.RHPONT_APUR_AGRUP x where x.codigo_empresa = '0001' and x.tipo_apur = 'F' AND c_livre_selec01 = 2 AND id_agrup = 152123;
...
,X3.cod_cargo_efetivo
FROM(

SELECT 
L.DATA_USAR, L.DESCRICAO DESCRICAO_vw, L.DATA_EXTINCAO,
X2.* 
--comentado 4/7/23--,CASE WHEN X2.DATA_INICIO < X2.ULTIMA_DATA_INI_FOLHA THEN X2.ULTIMA_DATA_INI_FOLHA ELSE X2.DATA_INICIO END DATA_INICIO_SIT_PONTO --EMAIL (Re: Lançamento - À disposição - Carina Polastri BM 106753-0)
...
*/--fim comentado em 15/2/24

--inicio--- novo ajustte em 15/2/24 devido virada do ano
,CASE 
WHEN TRUNC(X2.DT_ULT_ALTER_USUA) >= --RODEI EM 11/2/24--TO_DATE('01/12/23','DD/MM/YY')
CASE WHEN EXTRACT(MONTH FROM X2.ULTIMA_DATA_INI_FOLHA) <> 1 THEN --CASE NOVO EM 15/2/24
TO_DATE('01/'||LPAD((EXTRACT(MONTH FROM TO_DATE(X2.ULTIMA_DATA_INI_FOLHA,'DD/MM/RRRR')-1)),2,0)||'/'||EXTRACT(YEAR FROM TO_DATE(X2.ULTIMA_DATA_INI_FOLHA,'DD/MM/RRRR')),'DD/MM/RRRR') 
ELSE 
TO_DATE('01/12/'||EXTRACT(YEAR FROM TO_DATE(X2.ULTIMA_DATA_INI_FOLHA,'DD/MM/RRRR')-1),'DD/MM/RRRR')
END
AND TRUNC(X2.DATA_INICIO) < TRUNC(X2.ULTIMA_DATA_INI_FOLHA) 
                                                                                                                                                            THEN X2.DATA_INICIO 
WHEN TRUNC(X2.DT_ULT_ALTER_USUA) < --RODEI EM 11/2/24--TO_DATE('01/12/23','DD/MM/YY')
CASE WHEN EXTRACT(MONTH FROM X2.ULTIMA_DATA_INI_FOLHA) <> 1 THEN --CASE NOVO EM 15/2/24
TO_DATE('01/'||LPAD((EXTRACT(MONTH FROM TO_DATE(X2.ULTIMA_DATA_INI_FOLHA,'DD/MM/RRRR')-1)),2,0)||'/'||EXTRACT(YEAR FROM TO_DATE(X2.ULTIMA_DATA_INI_FOLHA,'DD/MM/RRRR')),'DD/MM/RRRR') 
...
AND TRUNC(X2.DATA_INICIO) < TRUNC(X2.ULTIMA_DATA_INI_FOLHA) 
                                                                                                                                                           THEN X2.ULTIMA_DATA_INI_FOLHA 
ELSE X2.DATA_INICIO END DATA_INICIO_SIT_PONTO 
--fim- --novo ajustte em 15/2/24 devido virada do ano
...
CASE WHEN A.DATA_FIM_SITUACAO IS NULL THEN  TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')-- TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--
...
AND (SF.data_fim_vigencia IS NULL OR SF.data_fim_vigencia > TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')-- TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')-- TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')--
)
...
LEFT OUTER JOIN ARTERH.RHPARM_P_FERI PF ON PF.CODIGO_EMPRESA = FE.CODIGO_EMPRESA AND PF.CODIGO = FE.TIPO_FERIAS
LEFT OUTER JOIN ARTERH.RHPONT_SITUACAO SP ON SP.CODIGO = PF.SITUACAO_PONTO
WHERE
...
LEFT OUTER JOIN (SELECT A.* FROM ARTERH.RHPESS_CONTRATO A WHERE A.ANO_MES_REFERENCIA = (SELECT MAX(AUX.ANO_MES_REFERENCIA)FROM ARTERH.RHPESS_CONTRATO AUX WHERE AUX.CODIGO_EMPRESA = A.CODIGO_EMPRESA AND AUX.TIPO_CONTRATO = A.TIPO_CONTRATO AND AUX.CODIGO = A.CODIGO)
...
LEFT OUTER JOIN ARTERH.RHPESS_CONTRATO C ON C.CODIGO_EMPRESA = X.CODIGO_EMPRESA AND C.TIPO_CONTRATO = X.TIPO_CONTRATO AND C.CODIGO = X.CONTRATO
LEFT OUTER JOIN (SELECT * FROM ARTERH.RHINTE_ED_IT_CONV WHERE CODIGO_CONVERSAO = 'TEG4')TEG4 ON SUBSTR(TEG4.DADO_ORIGEM,37,40) = X.CODIGO AND SUBSTR(TEG4.DADO_ORIGEM,1,36)= C.COD_CUSTO_GERENC1||C.COD_CUSTO_GERENC2||C.COD_CUSTO_GERENC3||C.COD_CUSTO_GERENC4||C.COD_CUSTO_GERENC5||C.COD_CUSTO_GERENC6
WHERE 
...
(x.data_inicio between TO_DATE(vDT_INI_PERIODO,'DD/MM/YY') AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')-- TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--TO_DATE(vDT_INI_PERIODO,'DD/MM/YY') AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')
OR x.data_fim between TO_DATE(vDT_INI_PERIODO,'DD/MM/YY') AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--TO_DATE(vDT_INI_PERIODO,'DD/MM/YY') AND TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--
-------------------------INICIO-----PARTE NOVA EM 20/11/19
OR 
(trunc(X.DATA_INICIO) <= TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')--to_date(vDT_INI_PERIODO,'dd/mm/yy') --TO_DATE(vDT_INI_PERIODO,'DD/MM/YY')
AND trunc(X.DATA_FIM)>= TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')--to_date(vDT_FIM_PERIODO,'dd/mm/yy')--TO_DATE(vDT_FIM_PERIODO,'DD/MM/YY')
)
...
AND X2.CODIGO_EMPRESA IN (SELECT DADO_ORIGEM FROM ARTERH.RHINTE_ED_IT_CONV WHERE CODIGO_CONVERSAO = 'IFP4')--NOVO EM 7/11/23--('0001','0003','0013','0014')
...
--WHERE X3.LOCAL1_VW <> '000095'--EM 1/9/21 --TIRANDO A SMSA PARA O PRIMEIRO FECHAMENTO IFPONTO mas incluir os 6 locais pilotos em 13/9
/*
WHERE X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000000' AND X3.LOCAL3_VW ='000015' AND X3.LOCAL4_VW='000039' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000'
--95	0	15	39	0	0
OR X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000000' AND X3.LOCAL3_VW='000016' AND X3.LOCAL4_VW='000030' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000' 
--95	0	16	30	0	0
OR X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000000' AND X3.LOCAL3_VW='000020' AND X3.LOCAL4_VW='000027' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000' 
--95	0	20	27	0	0
OR X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000000' AND X3.LOCAL3_VW='000021' AND X3.LOCAL4_VW='000024' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000' 
--95	0	21	24	0	0
OR X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000002' AND X3.LOCAL3_VW='000002' AND X3.LOCAL4_VW='000007' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000' 
--95	2	2	7	0	0
OR X3.LOCAL1_VW = '000095' AND X3.LOCAL2_VW='000004' AND X3.LOCAL3_VW='000002' AND X3.LOCAL4_VW='000003' AND X3.LOCAL5_VW='000000' AND X3.LOCAL6_VW='000000'
*/

--where x3.contrato = '000000000807447'--'000000000731211' --'000000000923544'--testes
...
--POPULAR VARIAVEIS
data_inicial := to_date(c1.DATA_INICIO_SIT_PONTO,'DD/MM/YYYY'); --to_date('01/11/21','DD/MM/YY');--comentado em 10/8/20--
data_final := to_date(c1.DATA_FIM_SIT_PONTO,'DD/MM/YYYY');--to_date('30/11/21','DD/MM/YY');--comentado em 10/8/20--
...
IF vULT_DIA_FECHA_GERAL_FREQ <> C1.DATA_USAR 
OR C1.cod_cargo_efetivo in ('000000000001730','000000000001761','000000000001763','000000000001765','000000000001766','000000000001767' ,'000000000003339','000000000003340','000000000009730','000000000009731', '000000000001768')--NOVO EM 9/5/22 LOCAIS GUARDAS QUE DATA DATA DO LOCAL ESTA IGUAL A GERAL
THEN   
