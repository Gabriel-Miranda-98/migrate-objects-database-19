  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_SUMOB" 
-- *****************************************************************************
...
IS
  ...
  LINHA                      VARCHAR(2000) := '';
  DIRETORIO                  VARCHAR(50)   := '/trab/cdbp05/sirh/arquivos_sumob_ad';
  ARQUIVO_INC                VARCHAR(50)   := 'Inclusao_AD_SUMOB_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR(50)   := 'Alteracao_AD_SUMOB_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR(50)   := 'Exclusao_AD_SUMOB_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ERR                VARCHAR(50)   := 'Erro_AD_SUMOB_'     || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_TIPO_ARQUIVO             VARCHAR(01)   := 'I';
  V_CONTA_REGISTRO           INTEGER       := 0;

  --VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/SUMOB';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionários da SUMOB no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionários da SUMOB no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionários da SUMOB no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusões de funcionários da SUMOB';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alterações de funcionários da SUMOB';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusões de funcionários da SUMOB';
...
BEGIN
  ...
  FOR I IN (SELECT CNT.CODIGO AS MATRICULA,
                   ...
                   CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'sm'   || LOWER(SUBSTR(CNT.CODIGO,7,8))
                        WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'smes' || LOWER(SUBSTR(CNT.CODIGO,7,8))
                   ELSE
                     'NÃO DEFINIDA'
                   END LOGIN,
                   CNT.NOME      AS NOME,
