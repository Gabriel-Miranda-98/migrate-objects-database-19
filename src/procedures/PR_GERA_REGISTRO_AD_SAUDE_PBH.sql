  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_SAUDE_PBH" 
-- *****************************************************************************
...
IS
  ...
  --VARIAVEIS REFERENTE A GERACAO DE ARQUIVO TXT
  ID_ARQ                     UTL_FILE.FILE_TYPE;
  LINHA                      VARCHAR(2000) := '';
  DIRETORIO                  VARCHAR(50)   := '/trab/cdbp05/sirh/arquivos_pbh_ad';
  ARQUIVO_INC                VARCHAR(50)   := 'Inclusao_AD_SAUDE_PBH_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR(50)   := 'Alteracao_AD_SAUDE_PBH_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR(50)   := 'Exclusao_AD_SAUDE_PBH_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ERR                VARCHAR(50)   := 'Erro_AD_SAUDE_PBH_'     || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_TIPO_ARQUIVO             VARCHAR(01)   := 'I';
  V_CONTA_REGISTRO           INTEGER       := 0;

  --VARIAVEIS REFERENTE A GERACAO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/SAUDE';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionÃƒÂ¡rios da PBH-SAUDE no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionÃƒÂ¡rios da PBH-SAUDE no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionÃƒÂ¡rios da PBH-SAUDE no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusÃƒÂµes de funcionÃƒÂ¡rios da PBH-SAUDE';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alteraÃƒÂ§ÃƒÂµes de funcionÃƒÂ¡rios da PBH-SAUDE';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusÃƒÂµes de funcionÃƒÂ¡rios da PBH-SAUDE';
...
BEGIN
   FOR I IN (SELECT CNT.CODIGO AS MATRICULA,
                    ...
                    CASE WHEN NVL(CNT.C_LIVRE_SELEC02,0) IN (21,0) AND CNT.TIPO_CONTRATO <> '0015' THEN 'prct' || LOWER(SUBSTR(CNT.CODIGO,9,7))
                         WHEN NVL(CNT.C_LIVRE_SELEC02,0) =  32     AND CNT.TIPO_CONTRATO <> '0015' THEN 'przo' || LOWER(SUBSTR(CNT.CODIGO,9,7))
						 WHEN NVL(CNT.C_LIVRE_SELEC02,0) =  0      AND CNT.TIPO_CONTRATO  = '0015' THEN 'prsa' || LOWER(SUBSTR(CNT.CODIGO,9,7))
                    ELSE
                        'NAO DEFINIDA' 
                    END LOGIN,
                    CNT.NOME      AS NOME,
                    CG.DESCRICAO  AS CARGO_PGTO,
                    CASE WHEN NVL(CNT.C_LIVRE_SELEC02,0) IN (21,0) AND CNT.TIPO_CONTRATO <> '0015' THEN 'CONTRATOS SA'
                         WHEN NVL(CNT.C_LIVRE_SELEC02,0) =  32     AND CNT.TIPO_CONTRATO <> '0015' THEN 'ZOONOZES'
						 WHEN NVL(CNT.C_LIVRE_SELEC02,0) =  0      AND CNT.TIPO_CONTRATO  = '0015'  THEN 'SAUDE'
                    ELSE
                          'NAO DEFINIDA'
                    END EMPRESA,
                    TRIM(PES.CPF) AS CPF,
            ...
            AND   CNT.TIPO_CONTRATO      IN ('0015','0211','0212','0213','0214','0215')
   ...
   LOOP
    ...
    IF V_INSERE THEN
      ...
      INSERT INTO ARTERH.RHPBH_REGISTRO_AD   
                                    (ID_RHPBH_REGISTRO_AD,
                                     BM_MATRIC,
                                     NOME,
                                     CARGO,
                                     EMPRESA,
  ...
  AND    AD.EMPRESA            IN ('CONTRATOS SA','ZOONOZES','SAUDE');
  ...
  LOOP
    ...
    FOR ARQTXT IN(SELECT TRIM(BM_MATRIC)  ||';'|| TRIM(NOME)    ||';'|| TRIM(CARGO) ||';'|| TRIM(EMPRESA)    ||';'||
				  ...
				  AND   EMPRESA          IN ('CONTRATOS SA','ZOONOZES','SAUDE')
                  ORDER  BY BM_MATRIC)
    LOOP
  ...
  LOOP
    ...
    END CASE;
  END LOOP;
END PR_GERA_REGISTRO_AD_SAUDE_PBH;