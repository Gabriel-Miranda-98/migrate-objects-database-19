  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_SUGESP_LACUNAS_SANEAMENTO" (EMPRESA IN VARCHAR2, TIPO_CONTRATO IN VARCHAR2, BM IN VARCHAR2) AS 
--KELLYSSON 18/2/20
...
AND S.CODIGO_EMPRESA  = vEMPRESA AND S.TIPO_CONTRATO = vTIPO_CONTRATO AND CODIGO_CONTRATO = vBM 
--and trunc(A.DATA_INIC_SITUACAO) <= to_date(S.DATA_CORTE_SIT_FUNC_PONTO,'dd/mm/yy')
)
---------------------------------------------------------------------------FIM-------PARA SANEAMENTO--------------------------------------
...
and c.data_rescisao is null and c.CODIGO_EMPRESA IN ('0001','0098')-----------------------para todos bm ativos em geral
)k
WHERE k.lacunas <> 0
...
dbms_output.put_line('INSERT INTO RHCGED_ALT_SIT_FUN (ind_ef_retro_esocial, TEXTO_ASSOCIADO, LOGIN_USUARIO, DT_ULT_ALTER_USUA, CODIGO_EMPRESA, TIPO_CONTRATO, CODIGO, DATA_INIC_SITUACAO, COD_SIT_FUNCIONAL ) values (''N'', ''Registro criado automaticamente pela rotina correcao de lacunas para projeto saneamento da aposentadoria.'', ''script_ajuste_lacunas_saneamento'' ,sysdate,'''|| C1.CODIGO_EMPRESA ||''','''|| C1.TIPO_CONTRATO ||''','''|| C1.codigo|| ''', TO_DATE(''' || TRUNC(C1.DATA_FIM) || ''',''dd/mm/yy'')+1, ''1000''); COMMIT;'  );
INSERT INTO RHCGED_ALT_SIT_FUN (ind_ef_retro_esocial, TEXTO_ASSOCIADO, LOGIN_USUARIO, DT_ULT_ALTER_USUA, CODIGO_EMPRESA, TIPO_CONTRATO, CODIGO, DATA_INIC_SITUACAO, COD_SIT_FUNCIONAL ) values ('N', 'Registro criado automaticamente pela rotina correcao de lacunas para projeto saneamento da aposentadoria.', 'script_ajuste_lacunas_saneamento', sysdate, C1.CODIGO_EMPRESA, C1.TIPO_CONTRATO, C1.codigo, TO_DATE(TRUNC(C1.DATA_FIM),'dd/mm/yy')+1, '1000'); COMMIT;
dbms_output.put_line('EXECUTE SUGESP_ALT_SIT_FUNC_ULT_CONTRA('''||C1.CODIGO_EMPRESA ||''','''|| C1.TIPO_CONTRATO ||''','''|| C1.CODIGO ||''',''script_ajuste_lacunas_saneamento'' );');
SUGESP_ALT_SIT_FUNC_ULT_CONTRA(C1.CODIGO_EMPRESA, C1.TIPO_CONTRATO, C1.CODIGO, 'script_ajuste_lacunas_saneamento' );
...
dbms_output.put_line('UPDATE RHCGED_ALT_SIT_FUN SET LOGIN_USUARIO = ''script_ajuste_lacunas_saneamento'' , DT_ULT_ALTER_USUA = SYSDATE, data_FIM_situacao = to_date('''||trunc(C1.PROXIMA_DATA_INICIO) ||''',''dd/mm/yy'')-1'||' where CODIGO_EMPRESA = '''||C1.CODIGO_EMPRESA ||''' AND TIPO_CONTRATO = '''||C1.TIPO_CONTRATO ||''' AND CODIGO = '''|| C1.CODIGO ||''' AND TRUNC(data_inic_situacao) = to_date('''||trunc(C1.DATA_INICIO) ||''',''dd/mm/yy'')' ||';COMMIT;' );
UPDATE RHCGED_ALT_SIT_FUN SET LOGIN_USUARIO = 'script_ajuste_lacunas_saneamento' , DT_ULT_ALTER_USUA = SYSDATE, data_FIM_situacao = to_date(trunc(C1.PROXIMA_DATA_INICIO),'dd/mm/yy')-1 where CODIGO_EMPRESA = C1.CODIGO_EMPRESA AND TIPO_CONTRATO = C1.TIPO_CONTRATO AND CODIGO = C1.CODIGO AND TRUNC(data_inic_situacao) = to_date(trunc(C1.DATA_INICIO),'dd/mm/yy'); COMMIT;
...
dbms_output.put_line('UPDATE RHCGED_ALT_SIT_FUN SET LOGIN_USUARIO = ''script_ajuste_lacunas_saneamento'' , DT_ULT_ALTER_USUA = SYSDATE, data_FIM_situacao = NULL where CODIGO_EMPRESA = '''||C1.CODIGO_EMPRESA ||''' AND TIPO_CONTRATO = '''||C1.TIPO_CONTRATO ||''' AND CODIGO = '''|| C1.CODIGO ||''' AND TRUNC(data_inic_situacao) = to_date('''||trunc(C1.DATA_INICIO) ||''',''dd/mm/yy'')' ||';COMMIT;' );
UPDATE RHCGED_ALT_SIT_FUN SET LOGIN_USUARIO = 'script_ajuste_lacunas_saneamento' , DT_ULT_ALTER_USUA = SYSDATE, data_FIM_situacao = NULL where CODIGO_EMPRESA = C1.CODIGO_EMPRESA AND TIPO_CONTRATO = C1.TIPO_CONTRATO AND CODIGO = C1.CODIGO AND TRUNC(data_inic_situacao) = to_date(trunc(C1.DATA_INICIO),'dd/mm/yy');COMMIT;
dbms_output.put_line('EXECUTE SUGESP_ALT_SIT_FUNC_ULT_CONTRA('''||C1.CODIGO_EMPRESA ||''','''|| C1.TIPO_CONTRATO ||''','''|| C1.CODIGO ||''',''script_ajuste_lacunas_saneamento'' );');
SUGESP_ALT_SIT_FUNC_ULT_CONTRA(C1.CODIGO_EMPRESA, C1.TIPO_CONTRATO, C1.CODIGO, 'script_ajuste_lacunas_saneamento');
