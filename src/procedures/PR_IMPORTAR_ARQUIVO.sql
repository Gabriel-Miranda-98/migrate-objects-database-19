...
vLISTA_LOG LISTA_LOG := LISTA_LOG(null,null,null);
  ...
  PROJETO: PREVIDÊNCIA COMPLEMENTAR
  */
...
begin
    ...
    IF PIDENTIFICADOR_ARQUIVO NOT IN (ARQUIVO_MAG_PBH,ARQUIVO_CONCESSOES, ARQUIVO_BENEFICIARIOS, ARQUIVO_MOVIMENTOS, ARQUIVO_MOVIMENTOS_DIRF, ARQUIVO_TABELA_VALORES_PS) THEN
    ...
    */
    IF v_NOME_ARQUIVO IS NULL THEN
       v_NOME_ARQUIVO := 'NAO_DEFINIDO' || '_' || v_CODIGO_EMPRESA || '_' || v_TIPO_ARQUIVO || '_' || replace( v_DATA_CARGA,'/','') || '_' || v_ID_ARQUIVO;
    END IF;
    ...
    IF vROTULO_ARQUIVO IS NULL THEN
       vROTULO_ARQUIVO := v_CODIGO_EMPRESA || '_' || v_TIPO_ARQUIVO || '_' || replace( v_DATA_CARGA,'/','') || '_' || LPAD(v_ID_ARQUIVO, 10, '0');
    END IF;
