  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_BELO" 
-- *****************************************************************************
...
IS
  ...
  LINHA                      VARCHAR2(2000)  := '';
  DIRETORIO                  VARCHAR2(50)    := '/trab/cdbp05/sirh/arquivos_belotur_ad';
  ARQUIVO_INC                VARCHAR2(50)    := 'Inclusao_AD_BELO_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR2(50)    := 'Exclusao_AD_BELO_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR2(50)    := 'Alteracao_AD_BELO_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_CONTA_REGISTRO           INTEGER         := 0;
  V_TIPO_ARQUIVO             VARCHAR(1)      := 'I';

  --VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/BELOTUR';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionários da BELOTUR no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionários da BELOTUR no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionários da BELOTUR no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusões de funcionários da BELOTUR';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alterações de funcionários da BELOTUR';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusões de funcionários da BELOTUR';
...
BEGIN
  ...
  FOR I IN (SELECT 	CNT.CODIGO AS MATRICULA,
                    ...
                    CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'bl'   || SUBSTR(CNT.CODIGO,9,6)  
                         WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'bles' || SUBSTR(CNT.CODIGO,9,6)  
                    ELSE
                      'NÃO DEFINIDA'
                    END LOGIN,                                                 
                    CNT.NOME AS NOME,
		    ...
		    AND CNT.CODIGO_EMPRESA        = '0009'
		    AND CNT.TIPO_CONTRATO        IN ('0001','0003')
