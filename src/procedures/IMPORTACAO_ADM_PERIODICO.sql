  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."IMPORTACAO_ADM_PERIODICO" ( peCODIGO_EMPRESA IN VARCHAR2, peCPF IN VARCHAR2, peDT_REG_OCORRENCIA IN DATE, peDATA_CARGA IN DATE, 
                                                       peNATUREZA_EXAME IN VARCHAR2, peCODIGO_ATENDENTE IN VARCHAR2, peNOME_ATENDENTE_ATENDENTE IN VARCHAR2, peINSCRICAO_CONSELHO_ATENDENTE IN VARCHAR2, 
                                                       peUF_CONSELHO_ATENDENTE IN VARCHAR2, peCONSELHO_REGIONAL_ATENDENTE IN VARCHAR2 ) AS 
...
RHPESS_PESSOA.CODIGO_EMPRESA, RHPESS_PESSOA.COD_DEFICIENCIA, RHPESS_PESSOA_DEFICIENCIA.COD_DEFICIENCIA , SUGESP_IMPORTACAO_ADM_PER.LOGIN_CARGA, SUGESP_IMPORTACAO_ADM_PER.DATA_ASO, SUGESP_IMPORTACAO_ADM_PER.ANALISE_DADOS, SUGESP_IMPORTACAO_ADM_PER.ANALISE_DEFICIENCIA,
CASE WHEN SUGESP_IMPORTACAO_ADM_PER.COD_DEFICIENCIA_PRINCIPAL IS NOT NULL AND RHPESS_PESSOA.COD_DEFICIENCIA  IS NOT NULL AND
          SUGESP_IMPORTACAO_ADM_PER.COD_DEFICIENCIA_PRINCIPAL = RHPESS_PESSOA.COD_DEFICIENCIA THEN 'DEFICIENCIA PRINCIPAL IGUAL A DEFICIENCIA DE PESSOA'
     WHEN SUGESP_IMPORTACAO_ADM_PER.COD_DEFICIENCIA_PRINCIPAL IS NOT NULL AND RHPESS_PESSOA.COD_DEFICIENCIA  IS NOT NULL AND
          SUGESP_IMPORTACAO_ADM_PER.COD_DEFICIENCIA_PRINCIPAL <> RHPESS_PESSOA.COD_DEFICIENCIA THEN 'DEFICIENCIA PRINCIPAL DIFERENTE A DEFICIENCIA DE PESSOA'
...
WHERE CPF = peCPF AND DATA_CARGA = TRUNC(peDATA_CARGA) AND DT_REG_OCORRENCIA = TRUNC(peDT_REG_OCORRENCIA) AND CODIGO_EMPRESA = peCODIGO_EMPRESA AND NATUREZA_EXAME = peNATUREZA_EXAME AND STATUS_PROCESSAMENTO IS NULL AND DATA_PROCESSAMENTO IS NULL;
...
AND NATUREZA_EXAME    = peNATUREZA_EXAME
AND CODIGO_EMPRESA    = peCODIGO_EMPRESA 
AND CPF               = peCPF
AND DT_REG_OCORRENCIA = TRUNC(peDT_REG_OCORRENCIA) 
AND DATA_CARGA        = TRUNC(peDATA_CARGA)
ORDER BY NATUREZA_EXAME, CPF, DATA_CARGA, NUMERO_LINHA
)

LOOP
dbms_output.put_line('INICIO LOOP PERIODICO');
IF C1.NUMERO_LINHA = LINHA THEN
ANALISAR_FICHA := PR_VERIFICA_FICHA (C1.CODIGO_EMPRESA, C1.CPF, C1.DT_REG_OCORRENCIA, C1.DATA_CARGA, C1.NATUREZA_EXAME, C1.CODIGO_CONTRATO,C1.TIPO_CONTRATO,peCODIGO_ATENDENTE, peNOME_ATENDENTE_ATENDENTE, peINSCRICAO_CONSELHO_ATENDENTE, peUF_CONSELHO_ATENDENTE, peCONSELHO_REGIONAL_ATENDENTE,C1.CODIGO_PESSOA_ENCONTRADO);
END IF; 

dbms_output.put_line('ANALISAR_FICHA.VERIFICA: '||ANALISAR_FICHA.VERIFICA||' C1.ANALISA_FICHA: '||C1.ANALISA_FICHA||' peCODIGO_ATENDENTE: '||peCODIGO_ATENDENTE||' C1.DATA_PROCESSAMENTO: '||C1.DATA_PROCESSAMENTO||' C1.STATUS_PROCESSAMENTO: '||C1.STATUS_PROCESSAMENTO);
IF (ANALISAR_FICHA.VERIFICA = 1 OR C1.ANALISA_FICHA = 'FICHA INSERIDA') AND peCODIGO_ATENDENTE IS NOT NULL AND C1.DATA_PROCESSAMENTO IS NULL AND C1.STATUS_PROCESSAMENTO IS NULL THEN

PR_IMPORTA_EXAME (C1.CODIGO_EMPRESA, C1.CPF, C1.NUMERO_LINHA, C1.DT_REG_OCORRENCIA, C1.DATA_CARGA, C1.CODIGO_PESSOA_ENCONTRADO, C1.COD_EXAME, ANALISAR_FICHA.OCORRENCIA, peCODIGO_ATENDENTE, peNOME_ATENDENTE_ATENDENTE, peINSCRICAO_CONSELHO_ATENDENTE, peUF_CONSELHO_ATENDENTE);
