  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_CNTPBH" 
-- *****************************************************************************
...
-- Última alteração.....: 11/05/2023
-- Atividade realizada..: Foi implementado a regra do novo ingresso Logins com 8 Dígitos (SDM 3439046 )
...
IS
  ...
  V_INSERE                     BOOLEAN;  
  V_DT_RESCISAO_MES_ANTERIOR   DATE;  -- AQUI
  
    --VARIÁVEIS REFERENTE A GERAÇÃO DE ARQUIVO TXT
  ID_ARQ                     UTL_FILE.FILE_TYPE;
  LINHA                      VARCHAR(2000) := '';
  DIRETORIO                  VARCHAR(50)   := '/trab/cdbp05/sirh/arquivos_contratospbh_ad';
  ARQUIVO_INC                VARCHAR(50)   := 'Inclusao_AD_CNTPBH_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_ALT                VARCHAR(50)   := 'Alteracao_AD_CNTPBH_'|| TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  ARQUIVO_EXC                VARCHAR(50)   := 'Exclusao_AD_CNTPBH_' || TO_CHAR(sysdate,'DDMMYYYY')||'.txt';
  V_TIPO_ARQUIVO             VARCHAR(01)   := 'I';
  V_CONTA_REGISTRO           INTEGER       := 0;

  --VARIÁVEIS REFERENTE A GERAÇÃO DE EMAIL
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/CONTRATOS PBH';
  ...
  AV_MSG_SUBJECT_INC         VARCHAR(200) := 'Arquivo para incluir funcionÃƒÂ¡rios da CONTRATOS PBH no AD';
  AV_MSG_SUBJECT_ALT         VARCHAR(200) := 'Arquivo para alterar funcionÃƒÂ¡rios da CONTRATOS PBH no AD';
  AV_MSG_SUBJECT_EXC         VARCHAR(200) := 'Arquivo para excluir funcionÃƒÂ¡rios da CONTRATOS PBH no AD';
  AV_MSG_TEXT_INC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as inclusÃƒÂµes de funcionÃƒÂ¡rios da CONTRATOS PBH';
  AV_MSG_TEXT_ALT            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as alteraÃƒÂ§ÃƒÂµes de funcionÃƒÂ¡rios da CONTRATOS PBH';
  AV_MSG_TEXT_EXC            VARCHAR(200) := 'Senhores,'||CHR(13)||CHR(13)||'  Segue em anexo o Arquivo com as exclusÃƒÂµes de funcionÃƒÂ¡rios da CONTRATOS PBH';
...
BEGIN
   FOR I IN (SELECT CNT.CODIGO AS MATRICULA,
                    ...
                    'prcp' || LOWER(SUBSTR(CNT.CODIGO,9,7)) LOGIN_7, -- PADRAO ANTIGO JÁ EXISTENTE
					'prcp' || LOWER(SUBSTR(CNT.CODIGO,8,9)) LOGIN_8, -- PADRÃO NOVO INGRESSO
                    CNT.NOME          AS NOME,
                    CG.DESCRICAO      AS CARGO_PGTO,
                    'CONTRATOS PBH' AS EMPRESA,
                    TRIM(PES.CPF)       AS CPF,
                    ...
                    SIT.E_AFASTAMENTO,
                    CNT.CODIGO_EMPRESA,
                    TRIM(CNT.CODIGO_PESSOA) AS CODIGO_PESSOA,
                    CNT.TIPO_CONTRATO,
                    CNT.COD_UNIDADE1,
                    CNT.COD_UNIDADE2,
                    CNT.COD_UNIDADE3,
                    ...
                    CNT.COD_UNIDADE5,
                    CNT.COD_UNIDADE6,
                    NVL(CNT.C_LIVRE_SELEC02,0) AS C_LIVRE_SELEC02,
					...
					ADD_MONTHS(TRUNC(CNT.ANO_MES_REFERENCIA),-1) AS MES_ANTERIOR -- AQUI
            ...
            AND CNT.CODIGO IN ('00000000121052X','000000003120072','000000003124701', '000000009352342')
            )
  LOOP   
	-- VERIFICAR SE O LOGIN NA RHUSER_P_SIST FOI GERADO PELO NOVO PROCESSO DO ARTE COM PREFIXO + 8 NÚMEEROS ( O NOVO PROCESSO SETA O CAMPO C_LIVRE_OPCAO01 = S )
	SELECT COUNT(*) INTO V_EXISTE_LOGIN_8
	FROM ARTERH.RHUSER_P_SIST
	WHERE EMPRESA_USUARIO    = I.CODIGO_EMPRESA
	AND   TP_CONTR_USUARIO   = I.TIPO_CONTRATO
	AND   CONTRATO_USUARIO   = I.MATRICULA
    AND   CODIGO_SERV_AUTENT = 'PWS1' 
	AND   CODIGO_USUARIO     = USUARIO_LDAP
	AND   C_LIVRE_OPCAO01    = 'S';

	-- VERIFICAR SE O LOGIN CRIADO NO AD RHPBH_REGISTRO_AD É IGUAL A PREFIXO + 6 NÚMEROS 
    SELECT COUNT(*) INTO V_EXISTE_LOGIN_7
    FROM ARTERH.RHPBH_REGISTRO_AD AD
    WHERE AD.BM_MATRIC = I.LOGIN_7
	AND   AD.EMPRESA   = I.EMPRESA
	AND   AD.CPF	   = I.CPF;	    


	IF ((V_EXISTE_LOGIN_8 = 0) AND (V_EXISTE_LOGIN_7 = 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_8; END IF;
	IF ((V_EXISTE_LOGIN_8 > 0) AND (V_EXISTE_LOGIN_7 = 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_8; END IF;
	IF ((V_EXISTE_LOGIN_8 = 0) AND (V_EXISTE_LOGIN_7 > 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_7; END IF;
	IF ((V_EXISTE_LOGIN_8 > 0) AND (V_EXISTE_LOGIN_7 > 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_7; END IF;  
	...
	-- GERENCIA
    BEGIN      
	  SELECT ABREVIACAO INTO V_UNIDADE
	  FROM 	ARTERH.RHORGA_UNIDADE
	  WHERE COD_UNIDADE1      = I.COD_UNIDADE1
	...
	IF TRIM(V_TITULAR_UNIDADE) = 'NA' THEN
	  ...
	  IF TRIM(V_GERENTE_LOTACAO_ADM) <> 'NA' THEN
	    BEGIN
		  SELECT DISTINCT NVL(AD.BM_MATRIC,'NA') INTO V_LOGIN_GERENTE
	      FROM ARTERH.RHPBH_REGISTRO_AD AD
          ...
          -- NÃO PODERÁ FILTRAR A EMPRESA QDO FOR PBH,SAUDE E CONTRATOS AND   AD.EMPRESA   = I.EMPRESA
    ...
    FROM ARTERH.RHPBH_REGISTRO_AD AD
    WHERE AD.BM_MATRIC = V_LOGIN_PADRAO
    AND   AD.EMPRESA   = I.EMPRESA
    AND   AD.CPF       = I.CPF;
    ...
    ELSE --EXISTE NA TABELA RHPBH_REGISTRO_AD
      --BUSCA NOME, CARGO, SECRETARIA, UNIDADE, DATA_RESCISAO E IND_OPERACAO DO ÃƒÅ¡LTIMO REGISTRO PARA COMPARAÃƒâ€¡ÃƒÆ’O
      BEGIN -- AQUI
        ...
        AND   AD.ID_RHPBH_REGISTRO_AD = (SELECT MAX(A.ID_RHPBH_REGISTRO_AD)
                                         ...
                                         WHERE  A.BM_MATRIC = V_LOGIN_PADRAO
      ...
      ELSE
		...
		-- AQUI
		-- NOVA REGRA - 29/12/2022 
		-- VALIDAR SE O ÚLTIMO REGISTRO GRAVADO NA TABELA RHPBH_REGISTRO_AD ESTA PREENCHIDO E COMPARAR COM A INFORMAÇÃO DO LOOP
		IF V_INSERE THEN
		  IF ((V_ULT_DATA_RESCISAO IS NOT NULL) AND (I.DATA_RESCISAO IS NULL) AND (V_ULT_IND_OPERACAO = 'E')) THEN
            V_OPERACAO_JOIN := 'R';  
          ELSE
            V_OPERACAO_JOIN := 'A';
		  END IF;	
    ...
    IF V_INSERE THEN
      ...
      WHERE  LOWER(U.CODIGO_USUARIO) = V_LOGIN_PADRAO;

      IF V_QTD_REGISTRO = 0 THEN
	    BEGIN
          INSERT INTO ARTERH.RHUSER_P_SIST 
                                    (CODIGO_USUARIO,          GERA_LOG,                USA_JANELA_ACOMP,        DT_M_OPER_PROPRIO,       DATA_SISTEMA,
          ...
          DELETE ARTERH.RHUSER_RL_USR_GRP WHERE LOWER(CODIGO_USUARIO) = V_LOGIN_PADRAO;
		  
          INSERT INTO ARTERH.RHUSER_RL_USR_GRP (CODIGO_USUARIO, CODIGO_GRUPO, LOGIN_USUARIO, DT_ULT_ALTER_USUA)
          SELECT V_LOGIN_PADRAO, T.CODIGO_GRUPO, 'PROCEDURE_AD', SYSDATE
          ...
          INSERT INTO ARTERH.RHUSER_RL_USR_EMP (CODIGO_USUARIO, CODIGO_EMPRESA, LOGIN_USUARIO, DT_ULT_ALTER_USUA)
                                         VALUES(V_LOGIN_PADRAO,I.CODIGO_EMPRESA,'PROCEDURE_AD', SYSDATE);
          END IF;
          ...
          INSERT INTO ARTERH.RHUSER_USR_TPCONT (CODIGO_USUARIO, TIPO_CONTRATO, LOGIN_USUARIO, DT_ULT_ALTER_USUA)
                                         VALUES(V_LOGIN_PADRAO, I.TIPO_CONTRATO, 'PROCEDURE_AD', SYSDATE);
          END IF;
	    EXCEPTION
	      WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('ERRO AO GRAVAR O LOGIN ' || V_LOGIN_PADRAO || ' NA TABELA RHUSER_P_SIST DO ARTERH. '
 ...
 -- ******************************************************************************************************
  ...
  END LOOP;
  -- ******************************************************************************************************
  -- ******************** ENVIA ARQUIVO TXT DE INCLUSÃƒÆ’O E EXCLUSÃƒÆ’O POR E-MAIL *****************************
  -- ******************************************************************************************************
  FOR I IN 1..3
  LOOP
    CASE
    ...
    END CASE;
  END LOOP;
END PR_GERA_REGISTRO_AD_CNTPBH;