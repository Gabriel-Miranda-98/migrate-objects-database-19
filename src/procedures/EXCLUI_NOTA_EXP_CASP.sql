  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."EXCLUI_NOTA_EXP_CASP" (ano IN char, mes IN char, tp_folha IN char, ch_acesso IN varchar)
IS
...
*/
  ...
  vsql := 'SELECT NUMERO_MAPA mapa 
           FROM CASP.VW_NOTA_EXPLICATIVA_ARTERH@LK_ARTERH_CASP.PBH  
  ...
  OPEN mapas FOR vsql;
    ...
    LOOP
       FETCH mapas INTO vmapa;
       
       EXIT WHEN mapas%NOTFOUND; 
/*
       -- TRECHO PARA AVALIAÇÃO APENAS 
       IF vmapa = 201504010000005 THEN
          ...
          DBMS_OUTPUT.PUT_LINE('qtde: '||mapas%rowcount);
          CASP.SP_EXCLUIR_NOTA_EXPLIC_ARTERH@LK_ARTERH_CASP.PBH(vmapa,vchave,verro);
          EXIT;
       END IF;  
