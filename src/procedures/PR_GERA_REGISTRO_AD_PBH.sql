  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_GERA_REGISTRO_AD_PBH" 
-- *****************************************************************************
...
IS
  ...
  AV_NAME_FROM               VARCHAR(200) := 'Dep.RH/PBH';
...
BEGIN
  ...
  FOR I IN (SELECT  CNT.CODIGO AS MATRICULA, 
					...
					CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'pr'   || LOWER(SUBSTR(CNT.CODIGO,9,6))
						WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'pres' || LOWER(SUBSTR(CNT.CODIGO,9,6))
					ELSE
						'NÃO DEFINIDA'
					END LOGIN_6,
					CASE WHEN CG.DESCRICAO NOT LIKE '%ESTAG%' THEN 'pr'   || LOWER(SUBSTR(CNT.CODIGO,7,8))
						WHEN CG.DESCRICAO     LIKE '%ESTAG%' THEN 'pres' || LOWER(SUBSTR(CNT.CODIGO,7,8))
					ELSE
						'NÃO DEFINIDA'
					END LOGIN_8,					
					CNT.NOME      AS NOME,
			...
			AND   CNT.TIPO_CONTRATO      IN ('0001','0007','0216')
			...
			AND   CNT.SITUACAO_FUNCIONAL NOT IN ('5016','5017','1710','1715')             
			-- SITUAÇÃO ESTAGIÁRIO CURRICULAR OBRIGATÓRIO, PIA E APOSENTADO TRANSFERIDO ESTÁ SENDO TRATADA NA PROCEDURE DO PIA

            -- IMPLEMENTAÇÃO DE REGRA DEFINIDA NO E-MAIL: "APOSENTADOS EM DESLIGADOS NO AD" NA DATA 05/05/2021 
	        -- SEGUNDO O LUIZ CARLOS E O KELYSSON UM FUNCIONARIO SO É CONSIDERADO APOSENTADO QDO A DATA DE RESCISAO É PREENCHIDA
	        -- E A SITUAÇÃO FUNCIONAL PASSA PARA 1715. 
	        -- ENQUANTO ISSO NÃO ACONTECE O FUNCIONARIO ESTÁ NA FOLHA DE ATIVOS, INDEPENDENTE DA SITUAÇÃO FUNCIONAL			
			)
  LOOP
    ...
    SELECT COUNT(*) INTO V_EXISTE_LOGIN_6
    FROM ARTERH.RHPBH_REGISTRO_AD AD
    WHERE AD.BM_MATRIC = I.LOGIN_6
	AND   AD.EMPRESA   = I.EMPRESA
	AND   AD.CPF	     = I.CPF;	    


	IF ((V_EXISTE_LOGIN_8 = 0) AND (V_EXISTE_LOGIN_6 = 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_8; END IF;
	IF ((V_EXISTE_LOGIN_8 > 0) AND (V_EXISTE_LOGIN_6 = 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_8; END IF;
	IF ((V_EXISTE_LOGIN_8 = 0) AND (V_EXISTE_LOGIN_6 > 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_6; END IF;
	IF ((V_EXISTE_LOGIN_8 > 0) AND (V_EXISTE_LOGIN_6 > 0)) THEN V_LOGIN_PADRAO :=  I.LOGIN_6; END IF;
