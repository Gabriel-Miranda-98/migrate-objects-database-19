  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "PONTO_ELETRONICO"."PRR_VACINACAO_SMSA" (DATA_INICIO IN DATE, DATA_FIM IN DATE) AS
BEGIN
...
SELECT 
'CENARIO|CODIGO_EMPRESA|TIPO_CONTRATO|CODIGO_CONTRATO|DATA|CODIGO_SITUACAO|SITUACAO_PONTO|REFERENCIA|LOGIN_USUARIO|DT_ULT_ALTER_USUA|TIPO_APURACAO|EMPRESA|TIPO_CONT|CONTRATO|DATA_LANC|CONTADOR|EMPRESA|TIPO_CONTR|MATRICULA|NOME|DATA|TIPO_SOBREAVISO|PREVISTO|REALIZADO|COD_CARGO_EFETIVO|CARGO_EFETIVO|TIPO_LOCAL_LOTACAO|COD_CG1|COD_CG2|COD_CG3|COD_CG4|COD_CG5|COD_CG6|LOCAL_LOTADO|vCHAVE_SAV1_PARCIAL|PREVIA_PROCESSO|vCHAVE_SAV1_FINAL|RESULTADO_FINAL_PROCESSAMENTO|vQTD_LOCAIS_PODE_VERBA|MARCACAO_ENTRADA|ORIGEM_MARCACAO_ENTRADA|EFETIVADO_ENTRADA|DADOS_MARCACAO_ENTRADA|MARCACAO_SAIDA|ORIGEM_MARCACAO_SAIDA|EFETIVADO_SAIDA|DADOS_MARCACAO_SAIDA|v1_LOCAL_PBH_MARCACAO_ENTRADA|v1_LOCAL_PBH_MARCACAO_SAIDA'
AS LINHA FROM DUAL UNION ALL

--SELECT X2.CENARIO, COUNT(1)QUANT FROM (  --SINTETICO
SELECT X2.CENARIO ||'|'||X2.CODIGO_EMPRESA||'|'||X2.TIPO_CONTRATO||'|'||X2.CODIGO_CONTRATO||'|'||X2.DATA||'|'|| X2.CODIGO_SITUACAO||'|'|| X2.SITUACAO_PONTO||'|'||X2.REFERENCIA||'|'||X2.LOGIN_USUARIO||'|'||X2.DT_ULT_ALTER_USUA||'|'||X2.TIPO_APURACAO||'|'||X2.EMPRESA||'|'|| X2.TIPO_CONTR||'|'|| X2.MATRICULA||'|'||TO_DATE(SUBSTR(X2.DATA_ESPELHO,1,8),'DD/MM/RRRR')||'|'||X2.CONSIDERACOES AS LINHA

FROM(
SELECT 

CASE
WHEN SP.CODIGO_EMPRESA IS NOT NULL AND X.CAMPO_4 = '*GRAVANDO SITUACAO DE PONTO COM VALIDACAO INTEGRAL TABELA SAV4 CONSIDERANDO MARCACOES: 0578-HORA EXTRA - CAMPANHA COVID-19' 
THEN '1-GEROU SIT PONTO, COM TODAS VALIDACOES'

WHEN SP.CODIGO_EMPRESA IS NOT NULL AND X.CAMPO_4 <> '*GRAVANDO SITUACAO DE PONTO COM VALIDACAO INTEGRAL TABELA SAV4 CONSIDERANDO MARCACOES: 0578-HORA EXTRA - CAMPANHA COVID-19' 
THEN '3-GEROU SIT PONTO, MAS NAO DETALHADO O MOTIVO'

WHEN SP.CODIGO_EMPRESA IS NULL AND X.CAMPO_4 = '*ATENCAO PESSOA COM CARGO COMISSINADO OU FUNCAO PUBLICA, NAO PODE SER PAGO A VERBA OU VERIFICAR COM A SMSA' 
THEN '2-NAO GEROU SIT PONTO, PESSOA COM CARGO COMISSINADO OU FUNCAO PUBLICA, NAO PODE SER PAGO A VERBA OU VERIFICAR COM A SMSA'
...
AND TRUNC(D.DT_ULT_ALTER_USUA) = TRUNC(SYSDATE)--TO_DATE('11/12/2021','DD/MM/YYYY')--
and d.codigo_situacao IN ('0578')
...
AND CAMPO_1 ='PRG11_CAMPANHA_VACINACAO'
...
INSERT INTO PONTO_ELETRONICO.IFPONTO_FECHAMENTO_LOG_RELAT (DATA_DADOS, CAMPO_1, CONSIDERACOES)VALUES(SYSDATE,'PRR_VACINACAO_SMSA',C1.LINHA);COMMIT;
