  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "ARTERH"."PR_REVERTER_PROCESSA_ARQUIVO" (PID_ARQUIVO IN NUMBER) AS

  vID_ARQUIVO NUMBER;
  vSITUACAO_ARQUIVO CHAR(2);
  vTIPO_ARQUIVO CHAR(4);
  vLINHA_ATUALIZADA NUMBER;
  vLOGIN_USUARIO VARCHAR2(40):= 'IMPORT_REVERTER';
...
BEGIN
    BEGIN
         ...
         CASE WHEN vSITUACAO_ARQUIVO = '07' THEN raise_application_error (-20002,'O ARQUIVO INFORMADO ESTA COM PROCESSAMENTO JA CONCLUIDO E NAO PODE SER MAIS PROCESSADO.');
              ...
              WHEN vSITUACAO_ARQUIVO in ('02','04','06') THEN raise_application_error (-20002,'O ARQUIVO INFORMADO JA ESTA EM PROCESSAMENTO E NAO PODE HAVER PROCESSAMENTOS COMCOMITANTES PARA O MESMO ARQUIVO.');
              ELSE
                  NULL;
         END CASE;
    ...
    EXCEPTION
    WHEN OTHERS THEN
       raise_application_error (-20002,'NAO FOI POSSIVEL REVERTER O PROCESSAMENTO DO ARQUIVO COM O ID_ARQUIVO INFORMADO.' || 'ENCONTRADO ERRO - '||SQLCODE||' -ERROR- '||SQLERRM);
    END;
    COMMIT;
END;