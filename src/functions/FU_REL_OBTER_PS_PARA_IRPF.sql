  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "ARTERH"."FU_REL_OBTER_PS_PARA_IRPF" (
...
case when co.tipo_registro = 'TPSE' then (select max(data_nascimento) from rhpess_pessoa where codigo_empresa = 
co.codigo_empresa and cpf = substr(co.cnpj_cpf_benefic,4))
	else
		co.data_nascimento
	end data_nascimento,  
f.razao_social,
...
co.codigo_empresa = f.codigo_empresa and
...
20, ' ')|| CHR(10);
    vtexto := vtexto || rpad(c2.relacao, 29, ' ') || LPAD(trim(TO_CHAR
(C2.VALOR_ANUAL,'9G999G999G999G990D99','NLS_NUMERIC_CHARACTERS = '',.''')),11, ' ')|| CHR(10);
